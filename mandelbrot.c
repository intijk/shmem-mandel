...
int main(int argc, char const *argv[])
{
		...
		//row number for every PE to tackle;
		...
		for(i=0;i<rowPerP;i++){
				for(j=0;j<width;j++){
						...
						if(cabs(z)>2){
							/*
								taskB[i*widthStep+j*nC + 0]=k+64%256;
								taskB[i*widthStep+j*nC + 1]=200-(k%256);
								taskB[i*widthStep+j*nC + 2]=(105+k%256)%256;
								*/
		...
		//collect data from different PEs
		if(me<npes-1){
